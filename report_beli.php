...
<div class="col-md-12">
 ...
 <!-- form start -->
            <form class="form-horizontal" method="post" action="report_beli.php" >
            <div class="row">
...
if($bulan == null || $bulan == "" ){
        ...
        $sqla="SELECT no, COUNT( * ) AS totaldata FROM $forward where LEFT(tglbeli,4) like '%$tahun%' AND MID(tglbeli,6,2) like '%$bulan%' or kasir like '%$search%'";
      }
...
?>
    ...
    if ($bulan != null || $bulan != "") {
        ...
        $query1="SELECT * FROM  beli  where YEAR(tglbeli) like '%$tahun%' AND MONTH(tglbeli) like '%$bulan%' order by no limit $rpp";
...
$jmlbeli=$row1['tot_beli'];
  ...
  <td>
  <?php if ($chmod >= 3 || $_SESSION['jabatan'] == 'admin') { ?>
    <button type="button" class="btn btn-info btn-xs no-print" onclick="window.location.href='stok_detail?id=2&trx=2&nota=<?php  echo $fill['nota']; ?>'">Detail</button>
 <?php } else {}?>
